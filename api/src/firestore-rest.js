...
async function getDocument({ serviceAccountRaw, collection, docId }) {
  const serviceAccount = parseServiceAccount(serviceAccountRaw);
...
async function updateDocument({ serviceAccountRaw, collection, docId, fields }) {
  const serviceAccount = parseServiceAccount(serviceAccountRaw);
  const accessToken = await getAccessToken(serviceAccountRaw);
  const projectId = serviceAccount.project_id;
  const documentName = `projects/${projectId}/databases/(default)/documents/${collection}/${docId}`;
...
async function upsertRecords({ serviceAccountRaw, collection, records }) {
  if (!records.length) {
    return 0;
  }
...
async function commitRecords(options) {
